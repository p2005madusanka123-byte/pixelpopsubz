...
import { Download, Send, CheckCircle2, Zap, Loader2 } from 'lucide-react';
...
export default function MovieDownloadSection({ subtitles, telegramLinks, movieTitle }: MovieDownloadSectionProps) {
  // Track unlock states per item
  ...
  // Active loading / downloading states
  ...
  const handleSubtitleClick = async (sub: any) => {
    if (!unlockedSubs[sub.id]) {
      ...
      try {
        await downloadSubtitleDirectly(sub.fileUrl, sub.fileName || `${movieTitle}.srt`, sub.id);
  ...
  return (
    <div className="space-y-8">
        ...
        <section className="space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="text-base font-black flex items-center gap-2" style={{ color: 'var(--color-text)' }}>
              ...
              Sinhala Subtitle Downloads (.SRT)
            </h2>
      ...
      {/* ── Telegram Links Download Section (Matching Full-Size Layout) ── */}
                  ...
                  <div className="flex items-center gap-3.5 min-w-0">
                    ...
                    <div className="min-w-0">
                      <p className="text-xs sm:text-sm font-bold text-white truncate group-hover:text-sky-300">
                        {movieTitle} {link.quality ? `— ${link.quality}` : ''}
                      </p>
                      <p className="text-[11px] text-neutral-400 mt-0.5 flex items-center gap-2">
